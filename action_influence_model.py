...
class ActionInfluenceModel:
        ...
        for key in unique_functions:
            if key[1] in action_influence_dataset:
                ...
                for x_feature in unique_functions[key]:
                    x_data.append((np.array(action_influence_dataset[key[1]]['state'])[
                                  :, [x_feature]]).flatten())  # instead of x_feature we want X-feature AND key[0]

                x_feature_cols = [
                    tf.feature_column.numeric_column(
                        str(i)) for i in range(
                        len(x_data))]  # we might want to change this

                y_data = np.array(action_influence_dataset[key[1]]['next_state'])[
                    :, key[0]]  # get new values of key[0]
                lr = tf.estimator.LinearRegressor(
                    feature_columns=x_feature_cols, model_dir='scm_models/linear_regressor/' + str(key[0]) + '_' + str(key[1]))
                structural_equations[key] = {
                    'X': x_data, 'Y': y_data, 'function': lr, }
    ...
    def _train_structural_equations(self):
        for key in self.structural_equations:
            self.structural_equations[key]['function'].train(
                input_fn=self.get_input_fn(
                    self.structural_equations[key],
                    num_epochs=None,
                    n_batch=128,
                    shuffle=False),
                steps=1000)

    def _generate_action_influence_dataset(
            self, state_set, action_set, next_state_set, data_set):
        action_influence_dataset = {}

        for idx, action in enumerate(action_set):
            if action in action_influence_dataset:
                action_influence_dataset[action]['state'].append(
                    state_set[idx])
                action_influence_dataset[action]['next_state'].append(
                    next_state_set[idx])
            else:
                action_influence_dataset[action] = {
                    'state': [], 'next_state': []}
                action_influence_dataset[action]['state'].append(
                    state_set[idx])
                action_influence_dataset[action]['next_state'].append(
    ...
    def predict_from_scm_given_action(self, structural_equations, s, a):
        ...
        for node in nodes:
            ...
            pred = structural_equations[key]['function'].predict(
    ...
    def process_explanations(
        ...
        why_explanations[(agent_step,
                          action)] = {'state': state_set[agent_step],
                                      'why_exps': generate_why_explanations(state_set[agent_step],
                                                                            action,
                                                                            agent_step,
                                                                            causal_graph2,
                                                                            structural_equations)}
    ...
    def generate_why_explanations(
        ...
        actual_action_edge_list = get_edges_of_actions(
            actual_action, causal_graph)
        all_actual_causal_chains_from_action = get_causal_chains(
            sink_nodes, actual_action_edge_list, causal_graph)
        # print(all_actual_causal_chains_from_action)
        action_chain_list = get_action_chains(
        ...
        for i in range(len(all_actual_causal_chains_from_action)):
            ...
            for j in range(len(all_actual_causal_chains_from_action[i])):
                for k in range(
                        len(all_actual_causal_chains_from_action[i][j])):
            ...
            min_tuple_actual_state = get_minimally_complete_tuples(
                all_actual_causal_chains_from_action[i], actual_state)
            min_tuple_optimal_state = get_minimally_complete_tuples(
                all_actual_causal_chains_from_action[i], optimal_state)
            explanation = explanations.cartpole_generate_why_text_explanations(
                min_tuple_actual_state, min_tuple_optimal_state, actual_state, actual_action)
            print(explanation)
            print("\n")
    ...
    def generate_counterfactual_explanations(
        ...
        actual_state = {k: actual_state[k] for k in range(len(actual_state))}
        sink_nodes = get_sink_nodes(causal_graph)
        counter_action_edge_list = get_edges_of_actions(
            counterfactual_action, causal_graph)
        actual_action_edge_list = get_edges_of_actions(
            actual_action, causal_graph)

        all_counter_causal_chains_from_action = get_causal_chains(
            sink_nodes, counter_action_edge_list, causal_graph)
        all_actual_causal_chains_from_action = get_causal_chains(
            sink_nodes, actual_action_edge_list, causal_graph)
        action_chain_list = get_action_chains(
            counterfactual_action,
            all_counter_causal_chains_from_action,
            causal_graph)
        ...
        for i in range(len(all_counter_causal_chains_from_action)):
            ...
            for j in range(len(all_counter_causal_chains_from_action[i])):
                for k in range(
                        len(all_counter_causal_chains_from_action[i][j])):
        ...
        for actual_chains in all_actual_causal_chains_from_action:
            for counter_chains in all_counter_causal_chains_from_action:
                for counter_states in counterfactual_state_set:
                    contrast_tuple = get_minimal_contrastive_tuples(
                        actual_chains, counter_chains, actual_state, counter_states)
                    contrastive_exp.add(
                        explanations.cartpole_generate_contrastive_text_explanations(
                            contrast_tuple, actual_action))
    ...
    def predict_node_scm(self, node, action, structural_equations):
        ...
        pred = structural_equations[key]['function'].predict(input_fn=get_input_fn(
            structural_equations[key], num_epochs=1, n_batch=128, shuffle=False))
    ...
    def get_minimal_contrastive_tuples(
            self,
            actual_chain,
        ...
        actual_minimally_complete_tuple = get_minimally_complete_tuples(
            actual_chain, actual_state)
        counterfactual_minimally_complete_tuple = get_minimally_complete_tuples(
            counterfactual_chain, counterfactual_state)
        min_tuples = np.sum(
            np.array([list(k) for k in list(actual_minimally_complete_tuple.values())]))
        tuple_states = set([k[0] for k in min_tuples])

        counter_min_tuples = np.sum(np.array(
            [list(k) for k in list(counterfactual_minimally_complete_tuple.values())]))
        counter_tuple_states = set([k[0] for k in counter_min_tuples])
        counter_tuple_states.difference_update(tuple_states)

        contrastive_tuple = {
            'actual': {
                k: actual_state[k] for k in counter_tuple_states}, 'counterfactual': {
                k: counterfactual_state[k] for k in counter_tuple_states}, 'reward': {
                k[0]: k[1] for k in actual_minimally_complete_tuple['reward']}}
            ...
            for chain in chain_list:
                ...
                for i in range(len(chain)):
                    if i == 0:
                        ...
                        action_chain.append(causal_graph.get_edge_data(
                            chain[i - 1], chain[i])[0]['action'])
    ...
    def get_edges_of_actions(self, action, causal_graph):
        return list(edge for edge in causal_graph.edges(
            data=True) if edge[2]['action'] == action)

    def get_sink_nodes(self, causal_graph):
        return list((node for node, out_degree in causal_graph.out_degree_iter(
        ) if out_degree == 0 and causal_graph.in_degree(node) > 0))
