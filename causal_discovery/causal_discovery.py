...
def get_data(path, num_datapoints=30000):
    ...
    data = open(path)
    data = np.loadtxt(data, delimiter=",")[:num_datapoints, :]
...
def main(args):
    # TODO: VarLiNGAM currently requires a different CSV structure than the
    # other algorithms (VarLiNGAM handles data in rows of state + action, but
    # the other algorithms need state + action + next state)
    data = get_data(args.path_to_dataset, num_datapoints=30000)
