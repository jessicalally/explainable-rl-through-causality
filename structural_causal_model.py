...
class StructuralCausalModel:
    ...
    def generate_why_explanations(
        ...
        # TODO: add something to the environment to make this relationship
        # clearer
        ...
        # Go through all the causal chains from the head nodes, skipping it if
        # it contains the action variable
        causal_chains = self.get_causal_chains(
            head_nodes, sink_nodes, causal_graph)

        # Predict the values of state variables in these causal chains using
        # the SCM at the action taken (Why A?)
    ...
    def generate_counterfactual_explanations(
        ...
        contrastive_tuple = self.get_minimal_contrastive_tuples(
            causal_chain, optimal_transition, counterfactual_transition)
        explanation = explanations.taxi_generate_contrastive_text_explanations(
            contrastive_tuple, actual_action)
    ...
    def get_minimal_contrastive_tuples(
        ...
        actual_minimally_complete_tuple = self.get_minimally_complete_tuples(
            causal_chain, optimal_transition)
        counterfactual_minimally_complete_tuple = self.get_minimally_complete_tuples(
            causal_chain, counterfactual_transition)
        ...
        contrastive_tuple = {
            'actual': {
                k: optimal_transition[k] for k in counter_tuple_states}, 'counterfactual': {
                k: counterfactual_transition[k] for k in counter_tuple_states}, 'reward': {
    ...
    # For a causal chain to exist, there needs to be a causal relationship that doesn't pass through the action variable
    # Since we assume the action depends on every state variable at time t, and affects every state variable at time t+1
    # Go through all the causal chains from the head nodes, skipping it if it
    # contains the action variable
